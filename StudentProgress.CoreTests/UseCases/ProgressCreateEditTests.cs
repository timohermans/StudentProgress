...
namespace StudentProgress.CoreTests.UseCases
{
  ...
  public class ProgressCreateEditTests : DatabaseTests
  {
    ...
    public async Task Updates_the_progress_update_successfully()
    {
      ...
      var command = (await new ProgressGetForCreateOrUpdate(getContext).HandleAsync(new ProgressGetForCreateOrUpdate.Query { GroupId = group.Id, StudentId = student.Id, Id = progress.Id })).Value.Command;
